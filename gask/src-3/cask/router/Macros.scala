...
object Macros {
      ...
      decorators match {
        ...
        case '{ $d: Decorator[outer, inner, _, _] } :: tail =>
          if (TypeRepr.of[inner] <:< prevOuter) {
            ...
            hasErrors = true
            report.error(
              s"required: gask.router.Decorator[_, ${prevOuter.show}, _, _]",
              d
            )
          }
  ...
  /** Convert a result to an HTTP response
    *
    * Note: essentially, all this method does is summon a `gask.internal.Conversion`
  ...
  def convertToResponse(using Quotes)(
    ...
    val conversionTpe = TypeRepr.of[gask.internal.Conversion].appliedTo(
    ...
    '{
      $conversion.asInstanceOf[gask.internal.Conversion[Any, Any]].f($result)
    }
  }
    ...
    val exprs0 = for(idx <- method.paramSymss.indices) yield {
      ...
      val exprs1 = for (param <- params) yield {
        ...
        val reader = decorator match {
          ...
          case None =>
            paramTpe match
              case '[gears.async.Async] => '{ AsyncParser[gears.async.Async] }
              case _ => decoTpe match
                case '[t] => '{ NoOpParser.instanceAnyRequest[t] }
        }
    ...
    '{
      EntryPoint[Cls, Any](
        ...
        invoke0 = (
          ...
          val parsedArgss: Seq[Seq[Either[Seq[gask.router.Result.ParamError], Any]]] =
