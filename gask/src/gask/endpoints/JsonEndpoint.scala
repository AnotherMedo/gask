...
import gask.model.Response.DataCompanion
import gask.model.{Request, Response}
import gask.router.{ArgReader, Result}

import collection.JavaConverters._

sealed trait JsReader[T] extends ArgReader[ujson.Value, T, gask.model.Request]
object JsReader{
  implicit def defaultJsReader[T: upickle.default.Reader]: JsReader[T] = new JsReader[T]{
    def arity = 1

    def read(ctx: gask.model.Request, label: String, input: ujson.Value): T = {
  ...
  implicit def paramReader[T: ParamReader]: JsReader[T] = new JsReader[T] {
    ...
    override def read(ctx: gask.model.Request, label: String, v: ujson.Value) = {
  ...
  def wrapFunction(ctx: Request, delegate: Delegate): Result[Response.Raw] = {
    val obj = for{
      json <-
        ...
        catch{case e: Throwable => Left(gask.model.Response(
