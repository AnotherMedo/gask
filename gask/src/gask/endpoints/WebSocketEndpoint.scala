...
import gask.router.Result
import gask.util.{Logger, Ws}
...
object WebsocketResult{
  implicit class Response[T](value0: gask.model.Response[T])
                            (implicit f: T => gask.model.Response.Data) extends WebsocketResult{
...
class websocket(val path: String, override val subpath: Boolean = false)
  extends gask.router.Endpoint[WebsocketResult, WebsocketResult, Seq[String], Request]{
