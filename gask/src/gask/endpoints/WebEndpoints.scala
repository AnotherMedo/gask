...
import gask.router.HttpEndpoint
import gask.model.{Request, Response}
import gask.router.{ArgReader, Result}
...
abstract class QueryParamReader[T]
  extends ArgReader[Seq[String], T, gask.model.Request]{
  def arity: Int
  def read(ctx: gask.model.Request, label: String, v: Seq[String]): T
}
object QueryParamReader{
  ...
  class SimpleParam[T](f: String => T) extends QueryParamReader[T]{
    def arity = 1
    def read(ctx: gask.model.Request, label: String, v: Seq[String]): T = f(v.head)
  }
  ...
  implicit def SeqParam[T: QueryParamReader]: QueryParamReader[Seq[T]] = new QueryParamReader[Seq[T]]{
    def arity = 1
    def read(ctx: gask.model.Request, label: String, v: Seq[String]): Seq[T] = {
  ...
  implicit def OptionParam[T: QueryParamReader]: QueryParamReader[Option[T]] = new QueryParamReader[Option[T]]{
    def arity = 1
    def read(ctx: gask.model.Request, label: String, v: Seq[String]): Option[T] = {
  ...
  implicit def paramReader[T: ParamReader]: QueryParamReader[T] = new QueryParamReader[T] {
    ...
    override def read(ctx: gask.model.Request, label: String, v: Seq[String]) = {
