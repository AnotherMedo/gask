...
import gask.endpoints.{WebsocketResult, WsHandler}
import gask.internal.{DispatchTrie, ThreadBlockingHandler, Util}
import gask.model._
import gask.router.{Decorator, EndpointMetadata, EntryPoint, Result}
...
import gask.internal.AsyncHandler
import gask.internal.AsyncAttachmentKey
/** A combination of [[gask.Main]] and [[gask.Routes]], ideal for small one-file
  * web applications.
  */
...
/** Defines the main entrypoint and configuration of the Gask web application.
  *
  * You can pass in an arbitrary number of [[gask.Routes]] objects for it to
...
abstract class Main {
  ...
  implicit def log: gask.util.Logger = new gask.util.Logger.Console()
  ...
  /** The handler that will be used to handle incoming requests. By default,
    ...
    *
    * When `gask.virtual-threads.enabled` is set to `true` and running with a
  ...
  protected def handlerExecutor(): Option[ExecutorService] = {
    for {
      config <- Option(true)
      if config
      executor <- Util.createDefaultGaskVirtualThreadExecutor
    } yield executor
  }

  def defaultHandler(using Async): HttpHandler = {
    ...
    cachedHandlerExecutor match {
      ...
      case Some(executor) => new AsyncHandler(executor, mainHandler)
    }
  }
  ...
  def main(args: Array[String]): Unit = {
    Async.blocking:
      ...
      val server = Undertow.builder
        ...
        .build
      server.start()
      // (1) This is kind of a spiritual duplicate of (2) but it showcases that the main function doesn't actually exit...
      // The problem must therefore be coming from the handler itself -> for some reason the handler finishes before waiting and kills child contexts
      // This is supported by the stack trace not showing main but rather saying the problem is coming somewhere around AsyncHandler
      println("Press <ENTER> to stop"); scala.io.StdIn.readLine()
      server.stop()

      // (2) register an on exit hook to stop the server
      ...
      // possibly wait for an interupt here
  }
...
object Main {
  ...
  /** property key to enable virtual thread support.
    */
  val VIRTUAL_THREAD_ENABLED = "gask.virtual-threads.enabled"
  ...
  class DefaultHandler(
    ...
    def handleRequest(exchange: HttpServerExchange): Unit = try {
      /* will probably need to make handlerExecutor default to using virtual threads otherwise 
      it will throw since the Blocking handler is created with nothing attached */
      val maybeAsync = exchange.getAttachment(AsyncAttachmentKey)
      if maybeAsync == null then
        throw new IllegalStateException("Async context not set on exchange!")
      given Async = maybeAsync
  ...
  def prepareDispatchTrie(
    ...
    val flattenedRoutes = for {
      routes <- allRoutes
      metadata <- routes.gaskMetadata.value
    } yield {
    ...
    val str =
      if (!debugMode)
        ...
        ErrorMsgs.formatInvokeError(
          routes,
          metadata.entryPoint.asInstanceOf[EntryPoint[gask.main.Routes, _]],
          e
        )
