...
/** What is known about a single endpoint for our routes. It has a [[name]],
  * [[argSignatures]] for each argument, and a macro-generated [[invoke0]] that
...
case class EntryPoint[T, C](
  ...
  def invoke(target: T, ctxs: Seq[C], paramLists: Seq[Map[String, Any]])(using
      Async
  ): Result[Any] = {
    ...
    if (
      missing.nonEmpty || (!argSignatures.exists(
        _.exists(_.reads.unknownQueryParams)
      ) && unknown.nonEmpty)
    ) {
