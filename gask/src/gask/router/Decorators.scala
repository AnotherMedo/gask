...
import gask.internal.Conversion
import gask.model.{Request, Response}

import gears.async.*
import gask.internal.AsyncAttachmentKey
...
/** A [[Decorator]] allows you to annotate a function to wrap it, via
  ...
  *
  * Calls to the wrapped function are done on the `delegate` parameter passed to
  * `wrapFunction`, which takes a `Map` representing any additional argument
  * lists (if any).
  */
trait Decorator[OuterReturned, InnerReturned, Input, InputContext]
    extends scala.annotation.Annotation {
  final type InputTypeAlias = Input
  type InputParser[T] <: ArgReader[Input, T, InputContext]
  final type Delegate =
    (InputContext, Map[String, Input]) => Result[InnerReturned]
  def wrapFunction(ctx: Request, delegate: Delegate): Result[OuterReturned]
  def getParamParser[T](implicit p: InputParser[T]) = p
}
object Decorator {

  /** A stack of [[Decorator]]s is invoked recursively: each decorator's
    * `wrapFunction` is invoked around the invocation of all inner decorators,
    * with the inner-most decorator finally invoking the route's
    * [[EntryPoint.invoke]] function.
    *
    * Each decorator (and the final `Endpoint`) contributes a dictionary of
    * name-value bindings, which are eventually all passed to
    * [[EntryPoint.invoke]]. Each decorator's dictionary corresponds to a
    * different argument list on [[EntryPoint.invoke]]. The bindings passed from
    * the router are aggregated with those from the `EndPoint` and used as the
    * first argument list.
    */
  def invoke[T](
      ctx: Request,
  ...
  )(using Async): Result[Any] = try {
    remainingDecorators match {
      case head :: rest =>
        head
          .asInstanceOf[Decorator[Any, Any, Any, Any]]
          .wrapFunction(
            ctx,
            (ictx, args) =>
              invoke(
      ...
      case Nil =>
        endpoint.wrapFunction(
          ctx,
          { (ictx: Any, endpointBindings: Map[String, Any]) =>
            val mergedEndpointBindings =
              endpointBindings ++ ctx.boundPathSegments.mapValues(
                endpoint.wrapPathSegment
              )
...
/** An [[HttpEndpoint]] that may return something else than a HTTP response,
  * e.g. a websocket endpoint which may instead return a websocket event handler
  */
trait Endpoint[OuterReturned, InnerReturned, Input, InputContext]
  ...
  /** [[HttpEndpoint]]s are unique among decorators in that they alone can bind
    * path segments to parameters, e.g. binding `/hello/:world` to `(world:
    * Int)`. In order to do so, we need to box up the path segment strings into
    * an [[Input]] so they can later be parsed by [[getParamParser]] into an
...
/** Annotates a Gask endpoint that returns a HTTP [[Response]]; similar to a
  * [[RawDecorator]] but with additional metadata and capabilities.
  */
trait HttpEndpoint[InnerReturned, Input]
    extends Endpoint[Response.Raw, InnerReturned, Input, Request]

class NoOpParser[Input, T, InputContext]
    extends ArgReader[Input, T, InputContext] {
  def arity = 1
...
object NoOpParser {
  implicit def instance[Input, T, InputContext]
      : NoOpParser[Input, T, InputContext] =
    new NoOpParser[Input, T, InputContext]
  implicit def instanceAny[T, InputContext]: NoOpParser[Any, T, InputContext] =
    new NoOpParser[Any, T, InputContext]
  implicit def instanceAnyRequest[T]: NoOpParser[Any, T, Request] =
    new NoOpParser[Any, T, Request]
}

class AsyncParser[T] extends ArgReader[Async, T, Request] {
  override def read(ctx: Request, label: String, input: Async): T =
