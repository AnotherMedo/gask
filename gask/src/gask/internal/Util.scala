...
object Util {
  ...
  import gask.util.Logger.Console.globalLogger
  ...
  def createVirtualThreadExecutor(executor: Executor): Option[ExecutorService] = {
    (for {
      factory <- Try(createVirtualThreadFactory("gask-handler-executor", executor))
      executor <- Try(createNewThreadPerTaskExecutor(factory))
    } yield executor).toOption
  }

  /**
   * Create a default gask virtual thread executor if possible.
   * */
  def createDefaultGaskVirtualThreadExecutor: Option[ExecutorService] = {
    for {
  ...
  private object CarrierThreadFactory extends ForkJoinPool.ForkJoinWorkerThreadFactory {
    ...
    override def newThread(pool: ForkJoinPool): ForkJoinWorkerThread = {
      ...
      carrierThread.setName("gask-carrier-thread-" + counter.incrementAndGet())
      carrierThread
    }
  }
