...
object RoutesEndpointsMetadata{
  ...
  implicit def initializeImpl[T: c.WeakTypeTag](c: blackbox.Context): c.Expr[RoutesEndpointsMetadata[T]] = {
    import c.universe._
    val router = new gask.router.Macros[c.type](c)
    ...
    val routeParts = for{
      ...
      val seqifyCall = annotObjectSyms
        .zip(annotPositions)
        .reverse
        .foldLeft[Tree](q"gask.router.EndpointMetadata.$seqify"){
          case (lhs, (rhs, pos)) => q"$lhs(${c.internal.setPos(q"$rhs", pos)})"
        }

      q"""{
        ..$declarations
        gask.router.EndpointMetadata(
    ...
    c.Expr[RoutesEndpointsMetadata[T]](q"""gask.router.RoutesEndpointsMetadata(..$routeParts)""")
  }
}
