...
class Macros[C <: blackbox.Context](val c: C) {
  ...
  def extractMethod(method: MethodSymbol,
    ...
    val argData = for(argListIndex <- method.paramLists.indices) yield{
      ...
      val argReader = argReaders.lift(argListIndex).getOrElse(q"gask.router.NoOpParser.instanceAnyRequest")
      ...
      val readArgSigs = for (
        ...
        if (arg.typeSignature.typeSymbol == definitions.RepeatedParamClass) c.abort(method.pos, "Varargs are not supported in gask routes")
        ...
        val argSig =
          q"""
          gask.router.ArgSig[$annotDeserializeType, $curCls, $docUnwrappedType, Any](
            ${arg.name.toString},
            ${docUnwrappedType.toString},
            $docTree,
            $defaultOpt
          )($argReader[$docUnwrappedType].asInstanceOf[gask.router.ArgReader[$annotDeserializeType, $docUnwrappedType, Any]])
        """

        val reader = q"""
          gask.router.Runtime.makeReadCall(
    ...
    val res = q"""
    gask.router.EntryPoint[$curCls, Any](
      ...
      (
        ...
        $argSigsSymbol: scala.Seq[scala.Seq[gask.router.ArgSig[Any, _, _, Any]]]
      ) =>
        gask.router.Runtime.validate(Seq(..${readArgs.flatten.toList})).map{
